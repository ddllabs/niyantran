-- 0009: document provenance metadata, chunk_commit, match_documents.
-- Spec: docs/specs/2026-09-20-document-rag-and-citations-design.md §D; ADRs 0003, 0004.
-- Plan: docs/plans/2026-09-21-document-rag-and-citations.md Task 3.
--
-- Down (manual):
--   drop function if exists public.match_documents(extensions.vector, int, uuid[], text);
--   drop function if exists public.chunk_commit(uuid, jsonb, text[]);
--   alter table public.documents drop column if exists metadata;

-- Provenance fields from Niyantran's export (section, doc_type, source_host,
-- licence_class, ocr_lang, ocr_quality, source_path, as_of …). Kept whole; never
-- interpreted by the server.
alter table public.documents add column if not exists metadata jsonb not null default '{}'::jsonb;

-- Reconcile one document's chunks (ADR 0004): delete what is not kept, update
-- the anchors of what is, insert what is new. Ids and embeddings of kept rows
-- survive. Service role only.
