...
export default function ModelPicker({ models = [], roles = [], value, onChange, open, onToggle }) {
  ...
  return (
    <div className="ai-v2-model">
      ...
      {open ? (
        <div className="ai-v2-model-menu" role="listbox" aria-label="Model">
          {roles.length ? (
            <>
              ...
              <div className="ai-v2-roles">
                {roles.map((r) => (
                  <button
                    ...
                    title={r.hint}
                    onClick={() => onChange?.({ ...value, modelId: r.model_id })}
                  >
                    {r.label}
                  </button>
                ))}
            ...
            <div key={g.vendor}>
              ...
              {g.items.map((m) => (
                <button
                  ...
                  type="button"
                  role="option"
                  aria-selected={m.model_id === picked?.model_id}
                  className={`ai-v2-model-opt${m.model_id === picked?.model_id ? ' on' : ''}`}
                  onClick={() => onChange?.({ ...value, modelId: m.model_id, effort: effortsFor(models, m.model_id).includes(value?.effort) ? value.effort : 'off' })}
                >
          ...
          <div className="ai-v2-efforts">
            {efforts.map((e) => (
              <button
                key={e}
                type="button"
                className={`ai-v2-effort${(value?.effort || 'off') === e ? ' on' : ''}`}
