...
|---|---|---|---|
| 0 | `brew install deno supabase/tap/supabase` → Deno 2.9.7, CLI 2.117.0; `supabase init`; `npm ci && npm run build` | Build passes with the two baseline warnings. `supabase link` refused without `supabase login` (owner action); migrations and deploys went through the Supabase connector instead, which the owner authorised. **Later the same day:** the owner ran `supabase login` and `supabase secrets set` (OPENROUTER_API_KEY, REFRESH_SECRET) in a desktop terminal; the supervisor linked the checkout (`"linked":true`) and repaired the migration history so the eight repo files map one-to-one onto the remote versions (every `supabase migration list` row paired; the nine connector-timestamped versions marked reverted). | 2026-09-21 |
...
| 4 | Migration applied; enabling a catalogued tool model ok (vendor derived); no-tools, unavailable and `openai/gpt-6-astra` (absent from the then-empty pricing table) refused with the trigger message — **correction after the first catalogue load:** `openai/gpt-6-astra` is a real tool-capable OpenRouter model, as are `google/gemini-3.5-flash-lite`, `google/gemini-3.7-flash`, `deepseek/deepseek-v4-flash` and `deepseek/deepseek-v4-pro`; only the frontend's unprefixed spellings are unusable, and those the lock-out still refuses; default on a disabled row refused; second default refused; role on disabled model refused; role on enabled ok; **vacuity:** trigger disabled inside a rolled-back subtransaction → placeholder insert succeeded; trigger still enabled after; zero residue | Pass. | 2026-09-21 |
| 5 | `deno test supabase/functions` 20 passed; **vacuity:** `requireUser` replaced by a constant → "401 without a token" failed by name, file restored byte-identical (sha256 match); deployed `health` v1 via connector; live: no token → 401 at the gateway, anon key → 401 `invalid or expired token` with CORS headers, preflight 204 | Pass. **Live 200 completed later the same day** with a session minted server-side for the developer test account `nter-auth-test+1789929957415@gmail.com` (owner-approved): `{ok:true, vector:true, pricing_rows:446, models_enabled:7, user_id:<that uid>}`, CORS header echoed for the dev origin, and the returned `user_id` equals the session's uid. | 2026-09-21 |
| 6 | Catalogue shape confirmed live (446 models, 378 with tools; pricing keys `prompt, completion, input_cache_read, input_cache_write, internal_reasoning`; `openai/text-embedding-3-small` present on `/api/v1/embeddings/models`); 6 Deno tests; **vacuity:** secret guard removed → "401 without the secret" failed; reconcile exercised live twice — dropped model marked unavailable and disabled, model that lost tools disabled, orphan role reported; `cron.job` row `0 */12 * * *` active; deployed v1; live: 401 no secret, 401 wrong secret, 405 GET | Pass. Two defects caught by the live test before commit and fixed: `now()` → `clock_timestamp()`, and explicit `revoke … from anon, authenticated` (default privileges had granted execute). **Completed later the same day:** Vault entry `refresh_secret` stored with the same value the owner set as `REFRESH_SECRET` (digests compared); the deployed function called with the header → 200 `{fetched: 446, upserted: 446, unavailable: 0}` in 3.7 s. The first catalogue load earlier that day went through pg_net → SQL mapper → the same RPC. | 2026-09-21 |
| 7 | 7 Deno tests; **vacuity:** admin gate removed → "403 for a non-admin" failed; `admin_models_upsert` live: merge keeps untouched fields, default swap proven with separate statements, placeholder refused, role on unknown model refused, bad kind refused, privileges anon=false authenticated=false service_role=true; deployed v1; live: no token 401, anon key 401 | Pass. **Completed later the same day** with the minted test session: GET as a plain user → 403 `admin only`; the test account promoted to `admin` for the run → GET 200 with 378 tool-capable catalogue rows; the owner-approved allowlist inserted through PUT — `google/gemini-3.5-flash-lite` (default), `google/gemini-3.7-flash`, `deepseek/deepseek-v4-flash`, `deepseek/deepseek-v4-pro`, `openai/gpt-6-astra`, `anthropic/claude-sonnet-5`, `google/gemini-2.5-flash-lite` (repair), all 200; four roles 200; the unprefixed frontend id `gemini-3.5-flash-lite` → 400 with the trigger message; test account demoted back to `user` afterwards. `AI_REPAIR_MODEL=google/gemini-2.5-flash-lite` set as a function secret. The owner's chosen admin, `nterai@gmail.com`, does not exist yet; it is promoted once the owner signs up. | 2026-09-21 |
